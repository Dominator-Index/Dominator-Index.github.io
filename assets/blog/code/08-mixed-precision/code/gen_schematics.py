"""第 08 篇示意图:fig-1 浮点格式位布局(范围 vs 精度),fig-2 混合精度训练循环(流量/存量)。"""
...
C_SIGN, C_EXP, C_MAN = S[5], S[4], S[0]  # 符号 品红 / 指数 琥珀 / 尾数 青
...
def arrow(x1, y1, x2, y2, color=GLOW, sw=2, marker="ag"):
    # 光晕 = 粗半透明底线(不用 filter,inkscape 安全)
...
def bits_row(x0, y0, name, n_exp, n_man, cell=23):
    """一行位布局:1 符号 + n_exp 指数 + n_man 尾数。返回 (svg, 总宽)。"""
...
def fig1():
    ...
    # bf16 = fp32 截断:虚线框住 fp32 的前 16 位
    keep = 1 + 8 + 7
