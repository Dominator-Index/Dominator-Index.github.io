...
**Megatron-LM** — `ColumnParallelLinear` / `RowParallelLinear` in `megatron/core/tensor_parallel/layers.py`, with the $$f/g$$ operators in `mappings.py`: `copy_to_tensor_model_parallel_region` = $$f$$ and `reduce_from_tensor_model_parallel_region` = $$g$$ — the names literally say copy-in / reduce-out.
