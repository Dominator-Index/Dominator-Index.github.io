...
**Bucket assignment** — `_compute_bucket_assignment_by_size` in `torch.distributed`. The first bucket defaults to 1 MB — deliberately tiny, so the last layer's gradients depart as early as possible; the rest default to `bucket_cap_mb` = 25 MB.
...
**`no_sync()`** — `torch/nn/parallel/distributed.py`. Disables the hooks' communication for the first gas−1 microbatches of gradient accumulation (§5).
